...
bool reader97(){
  ...
  if(can.available()){
    can.receive(canMsgI);
    if (canMsgI.id == 0x097){
      phy_curr = canMsgI.data[3] << 4;
      phy_curr = phy_curr | (canMsgI.data[4] >> 4);
...
void setupSensors(){
  ...
  readit.setInterval(5, hardread); //-------------------ciclo para levantar datos fisicos
}
...
void hardread(){
  float phy_torq = map(analogRead(pot0),4,1023,0,10); //modif max
  Tfilter(phy_torq);
  int phy_rpm=0;
  bool anslo = 0;
  byte gear = 2;
