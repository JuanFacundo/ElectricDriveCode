...
//-----------batery constants----------//
int n_cells = 48;                       //cant. de celdas
float vCell_max = 3.65;                 //tension max por celda
float vCell_min = 2.8;                  //tension min por celda
float I_max = 20.0;                     //corriente max
float I_min = 0.1;                      //corriente min

float rampaZone1 = 3.3;                 //donde la carga entra en la rampa
float rampaZone2 = 3.45;                //donde la carga sale de la rampa
...
void makePack(){
  ...
  if(GO){
    if(vChr <= rampaZone1 * n_cells){
      askI = I_max;
      lightColor = 0x04; //verde titila 
    } else if (vChr <= rampaZone2 * n_cells){
      askI = (pow(vChr-(rampaZone2*n_cells),4) / (pow((rampaZone1-rampaZone2)*n_cells,4) / (I_max - I_min) )) + I_min;
