...
//actuators
byte shiftNow = 0;
static const int DCL_air = 3; //porcentage of DCL that will be used as reference.
float holdTorq = 0;
//------/physical-variables----//
...
void fetchPack(){
  ...
  if((ask_torq == 0) && (digitalRead(thrOnly) == LOW)){
    ...
    ask_torq = filterTorq(ask_torq);
    gear = readGear();
  }

  ask_torq = PIDtorq(ask_torq, phy_torq);
