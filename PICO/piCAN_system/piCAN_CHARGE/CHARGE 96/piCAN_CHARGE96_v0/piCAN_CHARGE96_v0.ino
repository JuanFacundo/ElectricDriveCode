...
int rampaZone = 37;                     //porcentage left activates rampa
//----------/batery constants----------//
...
void CANreader(){
  ...
  if(can.available()){
    ...
    //*
    else if (canMsgIn.id == 0x18FFF4E5){
      receiveFF();
      printerFF();
    } //*/
    //*
...
void makePack(){
  ...
  int phyPor = (100*(vChr - vBat_min)) / (vBat_max - vBat_min);
  if(phyPor > 100){
    phyPor = 100;
  } else if (phyPor < 0){
    phyPor = 0;
...
void receiveFF(){
  ...
  aux = (canMsgIn.data[0] << 8);
  aux = aux | canMsgIn.data[1];
  vChr = (aux * 0.1);

  aux = (canMsgIn.data[2] << 8);
  aux = aux | (canMsgIn.data[3]);
  Ichr = (aux * 0.1) - 399.8;
}
...
//------printers-------//
void printerFF(){
  Serial.print("ID: 0x18FFF4E5  chr Volt: ");
  Serial.print(vChr);
  Serial.print("  chr I: ");
  Serial.println(Ichr);
}
