...
void setupSensors(){
  ...
  for (int i=16;i<=21;i++){
    pinMode(i,INPUT);  //pull up row - internal
  }
  pinMode(6,INPUT);  //push button aka allower
  pinMode(8,OUTPUT); //relay startstop
}
...
void StartStop() {
  if (!digitalRead(10) & !StartStopPress){
    StartStopPress = 1;
    if (!onoff){
      onoff = 1;
      digitalWrite(8,HIGH);
  ...
  } else if (digitalRead(10) & StartStopPress){
    StartStopPress = 0;
  }
}
