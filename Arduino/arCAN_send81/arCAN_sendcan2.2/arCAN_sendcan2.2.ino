...
void loop() {
  float phy_torq = map(analogRead(pot0),0,1023,-51,10); //modif max
  bool anslo = 0;
  byte gear = readGear();

  if (mcp2515.readMessage(&canMsg55) == MCP2515::ERROR_OK) {
    if (canMsg55.can_id == 0x055){
      int64_t hex_rpm = canMsg55.data[0] << 8;
      hex_rpm = hex_rpm | canMsg55.data[1];
